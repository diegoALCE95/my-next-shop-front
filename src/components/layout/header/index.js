...
const Header = ({ header }) => {
	...
	return (
		<>
			<Head>
				<title>{ `${siteTitle} - ${siteDescription}` || 'Nextjs WooCommerce' }</title>
							...
							<div className="text-sm font-medium uppercase lg:flex-grow">
								{ ! isEmpty( headerMenuItems ) && headerMenuItems.length ? headerMenuItems.map( menuItem => (
									<Link key={menuItem?.ID} href={ menuItem?.url ?? '/' }>
										<a className="block mt-4 lg:inline-block lg:mt-0 text-black hover:text-black mr-10" dangerouslySetInnerHTML={{ __html: menuItem.title }} />
									</Link>
								) ) : null }
							</div>
